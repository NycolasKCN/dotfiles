...
case "$SELECTION" in
	...
	"record selection")
		...
		notify-send -u low "Capture" "Starting capturing selected region"
		wf-recorder -a -b 30 -g "$(slurp)" -f "$VID" -y 2> /tmp/wf-rec-err.log & disown
		;;
	"record DP-1")
		echo "$VID" > /tmp/recording.txt
		notify-send -u low "Capture" "Starting capturing DP-1 screen"
		wf-recorder -a -b 30 -o DP-1 -f "$VID" -y 2> /tmp/wf-rec-err.log  & disown
		;;
	"record DP-2")
		echo "$VID" > /tmp/recording.txt
		notify-send -u low "Capture" "Starting capturing DP-2 screen"
		wf-recorder -a -b 30 -o DP-2 -f "$VID" -y 2> /tmp/wf-rec-err.log  & disown
		;;
	*)
		;;
esac
